...
- :class:`NodeFetchResult` — immutable model bundling the fetch specification, its resolved node tree,
  a :data:`~guffin.roam_node.NodesByUid` index of all fetched nodes, and the raw
  Datalog query result before :class:`~guffin.roam_node.RoamNode` parsing.
- :data:`NodeFetchResult_Placeholder` — flat list of :class:`~guffin.roam_node.RoamNode` records
  returned by all :class:`~guffin.roam_node_fetch.FetchRoamNodes` fetch methods.
- :func:`anchor_node` — return the :class:`~guffin.roam_node.RoamNode` in a
  :data:`~guffin.roam_network.NodeNetwork` that matches a :class:`NodeFetchAnchor`.
- :func:`anchor_tree` — return the subtree of a :data:`~guffin.roam_network.NodeNetwork`
...
from guffin.roam_primitives import UID_RE
from guffin.roam_tree import NodeTree


@enum.unique
class QueryAnchorKind(enum.Enum):
    """Discriminates the kind of anchor passed to :class:`~guffin.roam_node_fetch.FetchRoamNodes` fetch methods.
    ...
    def of(target: str) -> QueryAnchorKind:
        ...
        Returns:
            ...
            :data:`~guffin.roam_primitives.UID_RE`; :attr:`PAGE_TITLE` otherwise.
        """
...
class NodeFetchSpec(BaseModel):
    ...
    Attributes:
        ...
        include_node_tree: When ``True``, parse fetched pull-blocks into a
            :class:`~guffin.roam_node.RoamNode` model tree and populate
...
class NodeFetchResult(BaseModel):
    ...
    Attributes:
        ...
        anchor_tree: The :class:`~guffin.roam_tree.NodeTree` rooted at the fetch anchor.
        ...
        raw_result: The raw Datalog query result from the Local API before
            :class:`~guffin.roam_node.RoamNode` parsing.  Each outer list element is a
        ...
        network: All :class:`~guffin.roam_node.RoamNode` instances fetched by this result,
            as a flat :data:`~guffin.roam_network.NodeNetwork` list.  Empty when
    ...
    def network(self) -> NodeNetwork:
        """Return all fetched nodes as a flat :data:`~guffin.roam_network.NodeNetwork` list.
        ...
        Returns:
            A :data:`~guffin.roam_network.NodeNetwork` containing every
            :class:`~guffin.roam_node.RoamNode` in :attr:`nodes_by_uid`, or ``[]``
    ...
    def from_raw_result(
        ...
        Args:
            fetch_spec: The fetch specification used to perform the fetch.
            raw_result: The raw Datalog query result before :class:`~guffin.roam_node.RoamNode`
    ...
    def from_network(
        ...
        :class:`~guffin.roam_tree.NodeTree` rooted there, and builds a
        :data:`~guffin.roam_node.NodesByUid` index over all nodes in *network*.
...
def anchor_node(network: NodeNetwork, anchor: NodeFetchAnchor) -> RoamNode:
    ...
    Args:
        ...
        anchor: The fetch anchor whose :attr:`~NodeFetchAnchor.qualifier` string identifies
            the anchor node — matched against :attr:`~guffin.roam_node.RoamNode.uid`
            for :attr:`~QueryAnchorKind.NODE_UID` anchors, or against
            :attr:`~guffin.roam_node.RoamNode.title` for :attr:`~QueryAnchorKind.PAGE_TITLE`
            anchors.

    Returns:
        The matching :class:`~guffin.roam_node.RoamNode`.
...
def anchor_tree(network: NodeNetwork, anchor: NodeFetchAnchor) -> NodeNetwork:
    ...
    Returns:
        A :data:`~guffin.roam_network.NodeNetwork` containing the anchor node and
